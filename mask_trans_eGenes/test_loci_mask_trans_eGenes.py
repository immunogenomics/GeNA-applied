# Generates a single-cell object that drops information for all indicated genes 
# (relative to the discovery data object)
...
parser.add_argument("--trans_eGenes",type=str)
...
trans_eGenes = pd.read_csv(args.trans_eGenes, header = None).iloc[:,0].values
...
keep_vargenes = [i for i in np.arange(d.var.shape[0]) if d.var.index[i] not in trans_eGenes]
...
sc.tl.pca(d_mc, svd_solver='arpack')
...
print("nn graph")
sc.pp.neighbors(d_mc, use_rep = 'harmpca')
